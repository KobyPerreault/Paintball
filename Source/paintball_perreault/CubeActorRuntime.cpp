...
// Sets default values
ACubeActorRuntime::ACubeActorRuntime()
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	PrimaryActorTick.bCanEverTick = true;

	cube1 = CreateDefaultSubobject<URuntimeMeshComponentStatic>(TEXT("GeneratedCube"));
	RootComponent = cube1;

}

// Called when the game starts or when spawned
void ACubeActorRuntime::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void ACubeActorRuntime::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ACubeActorRuntime::PostActorCreated()
{
	Super::PostActorCreated();
	GenerateBoxMesh();
}

void ACubeActorRuntime::PostLoad()
{
	Super::PostLoad();
	GenerateBoxMesh();
}

void ACubeActorRuntime::GenerateBoxMesh()
{
	...
	cube1->CreateSectionFromComponents(0, 0, 0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, ERuntimeMeshUpdateFrequency::Infrequent, true);
}

void ACubeActorRuntime::CreateBoxMesh(FVector BoxRadius, TArray <FVector>& Vertices, TArray <int32>& Triangles, TArray <FVector>& Normals, TArray <FVector2D>& UVs, TArray <FRuntimeMeshTangent>& Tangents, TArray <FColor>& Colors)
{
	FVector BoxVerts[8];
	...
	Normals.Reset();
	Normals.AddUninitialized(NumVerts); Tangents.Reset();
